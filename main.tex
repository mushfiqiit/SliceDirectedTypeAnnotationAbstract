...
Specimin's stubbing replaces non-target method bodies and removes the code that constrains a field's type, its dereferences, null-guards (\texttt{if (x != null)}), and assignments in \emph{other} methods. Without that evidence, the model over-approximates and marks fields \texttt{@Nullable} defensively. Specimin can omit important field-usage context from the generated slice, which may lead the LLM to incorrect nullability inferences. We therefore extract the relevant field references from the original source and provide them separately as usage context. These go to the model as labelled, read-only evidence, not compilable code, giving it two complementary inputs: the compilable slice the checker also consumes, and textual usage evidence only the model sees. Since this evidence is just def/use information, none of this mechanism is specific to nullability.
